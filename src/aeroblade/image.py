...
def compute_reconstructions(
    ...
    if not (
        ...
        for images, paths in tqdm(
            ...
            #breakpoint()
            #for latent in zip(latents):
            #    #breakpoint()
            #    latent_tensor = latent[0][0]
            #    to_pil_image(latent_tensor).show()
            ...
            # save
            for reconstruction, path, latent in zip(reconstructions, paths, latents):
                reconstruction_path = output_dir / f"{Path(path).stem}.png"
                latent_path = output_dir /  f"{Path(path).stem}_latent.png" # Added

                to_pil_image(reconstruction).save(reconstruction_path)
                
                latent_tensor = latent[0][0] # added
                breakpoint()
                to_pil_image(latent_tensor).save() # added
